...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/16 07:59:15 by amabbadi          #+#    #+#             */
/*   Updated: 2025/01/22 22:49:21 by amabbadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*unprocessed[OPEN_MAX];
	char		*oneline;
	...
	if (fd < 0 || BUFFER_SIZE <= 0 \
		...
		return (NULL);
	oneline = NULL;
	if (main_loop(&unprocessed[fd], &oneline, fd) == -1)
	{
		free (unprocessed[fd]);
		unprocessed[fd] = NULL;
		return (NULL);
	}
	if (!oneline && unprocessed[fd] && *unprocessed[fd])
	{
		oneline = ft_strdup(unprocessed[fd]);
		free(unprocessed[fd]);
		unprocessed[fd] = NULL;
	}
	if (!oneline && unprocessed[fd])
	{
		free(unprocessed[fd]);
		unprocessed[fd] = NULL;
	}
