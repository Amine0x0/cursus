...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw_helpers2_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: amabbadi <amabbadi@student.1337.ma>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/29 19:30:00 by amabbadi          #+#    #+#             */
/*   Updated: 2026/01/13 01:58:22 by amabbadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_tex_mapping(t_wall_draw *wall, t_ray *ray, t_texture *texture)
{
	...
	step = STEP_FULL * texture->height / wall->line_height;
	wall->tex_pos = (wall->draw_start - wall->window_height / 2
...
{
	if (ray->side == 0)
		wall->wall_x = game->player.y + (wall->map_x - game->player.x
				+ (STEP_FULL - ray->step_x) / 2) / ray->dir_x * ray->dir_y;
	else
		wall->wall_x = game->player.x + (wall->map_y - game->player.y
				+ (STEP_FULL - ray->step_y) / 2) / ray->dir_y * ray->dir_x;
}
